...
serve(async (req) => {
  ...
  try {
    ...
    if (!pixelId) {
      console.log('[CAPI] ViewContent: Pixel ID not configured, using test mode');
      return new Response(
        JSON.stringify({ 
          success: true, 
          test_mode: true,
    ...
    if (isTestMode) {
      console.log('[CAPI] ViewContent: Running in TEST MODE');
      return new Response(
        JSON.stringify({ 
          success: true, 
          test_mode: true,
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      if (result.success) {
        console.log('[CAPI] ViewContent sent successfully:', result.result);
        return new Response(
          JSON.stringify({ 
            success: true, 
    ...
    console.error('[CAPI] ViewContent error after retries:', lastError);
    return new Response(
      JSON.stringify({ 
