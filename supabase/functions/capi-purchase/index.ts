/**
 * Supabase Edge Function - Meta Conversions API Purchase Event
...
serve(async (req) => {
  ...
  try {
    ...
    if (!pixelId) {
      console.log('[CAPI] Purchase: Pixel ID not configured, using test mode');
      return new Response(
        JSON.stringify({ 
          success: true, 
          test_mode: true,
          message: 'Purchase event logged (TEST MODE - Pixel ID not configured)',
          events_received: 1 
        }),
        { 
          status: 200, 
    ...
    if (isTestMode) {
      console.log('[CAPI] Purchase: Running in TEST MODE');
      return new Response(
        JSON.stringify({ 
          success: true, 
          test_mode: true,
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      if (result.success) {
        console.log('[CAPI] Purchase sent successfully:', result.result);
        return new Response(
          JSON.stringify({ 
            success: true, 
            events_received: result.result?.events_received || 0,
            event_id: result.result?.events_received?.[0]?.event_id || eventData.event_id,
          }),
          { 
            status: 200, 
            headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
    ...
    console.error('[CAPI] Purchase error after retries:', lastError);
    return new Response(
      JSON.stringify({ 
