...
serve(async (req) => {
  ...
  try {
    ...
    if (!pixelId) {
      console.log('[CAPI] PageView: Pixel ID not configured, using test mode');
      return new Response(
        JSON.stringify({ 
          success: true, 
          test_mode: true,
          message: 'PageView event logged (TEST MODE - Pixel ID not configured)',
          events_received: 1 
        }),
        { 
          status: 200, 
    ...
    if (isTestMode) {
      console.log('[CAPI] PageView: Running in TEST MODE');
      return new Response(
        JSON.stringify({ 
          success: true, 
          test_mode: true,
          message: 'PageView event logged (TEST MODE)',
          events_received: 1 
        }),
        { 
          status: 200, 
    ...
    if (user_data.phone) {
      hashedUserData.ph = [await hashPII(user_data.phone)];
    }
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      if (result.success) {
        console.log('[CAPI] PageView sent successfully:', result.result);
        return new Response(
          JSON.stringify({ 
            success: true, 
    ...
    console.error('[CAPI] PageView error after retries:', lastError);
    return new Response(
      JSON.stringify({ 
