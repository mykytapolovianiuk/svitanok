...
serve(async (req) => {
  ...
  try {
    ...
    if (!pixelId) {
      console.log('[CAPI] InitiateCheckout: Pixel ID not configured, using test mode');
      return new Response(
        JSON.stringify({ 
          success: true, 
          test_mode: true,
          message: 'InitiateCheckout event logged (TEST MODE)',
          events_received: 1 
        }),
        { 
          status: 200, 
    ...
    if (isTestMode) {
      console.log('[CAPI] InitiateCheckout: Running in TEST MODE');
      return new Response(
        JSON.stringify({ 
          success: true, 
          test_mode: true,
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      if (result.success) {
        console.log('[CAPI] InitiateCheckout sent successfully:', result.result);
        return new Response(
          JSON.stringify({ 
            success: true, 
    ...
    console.error('[CAPI] InitiateCheckout error after retries:', lastError);
    return new Response(
      JSON.stringify({ 
        error: 'Failed to send InitiateCheckout event',
        details: lastError 
      }),
      { 
        status: 500, 
